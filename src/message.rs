...
fn decodeMessage(bytes: &[u8]) -> Option<MessagePosition> {
    ...
    let str_slice = match std::str::from_utf8(&packet.type_){
        Ok(s) => s,
        Err(e) => {
            return None
        }
    ...
    let ac = String::from_utf8_lossy(&packet.ac[..]).trim_end_matches('\0').to_string();
    let ps = f64::from_be_bytes(packet.position.to_be_bytes());
    let symbol = String::from_utf8_lossy(&packet.product[..]).trim_end_matches('\0').to_string();

    Some(MessagePosition { ac, ps, symbol })
}

fn encodeMessage(message: &MessagePosition) -> [u8; std::mem::size_of::<Packet>()] {
    ...
    packet.type_.copy_from_slice(b"0001");
